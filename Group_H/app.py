...
def room_detail(room_id):
    ...
    # 1. 房间基本信息（不变，只是把 funcName 取出来方便后面用）
    room = db.execute("""
        SELECT r.*, f.floorNo, w.wingName, b.name AS buildingName,
               rf.functionCode, rf.name AS funcName
        FROM room r
        JOIN floor f ON r.floorId = f.floorId
        JOIN wing w ON f.wingId = w.wingId
    ...
    # 关键修改：实时根据房型规则查询 Amenities（不再依赖 room_has_fixture）
    fixtures = db.execute("""
        SELECT DISTINCT fix.fixtureId, fix.name
        FROM room_has_function rhf
        JOIN fixture_by_function fbf ON rhf.functionCode = fbf.functionCode
        JOIN room_fixture fix ON fbf.fixtureId = fix.fixtureId
        WHERE rhf.roomId = ?
        ORDER BY fix.fixtureId
    """, (room_id,)).fetchall()

    # 如果万一某房间还没分配 functionCode（理论上不会），给个保底
    if not fixtures and room and room['baseRate'] is not None:
        fallback_code = 'MTG' if room_id >= 2000 else ('STE' if room['baseRate'] >= 750 else 'SLP')
        fixtures = db.execute("""
            SELECT fix.fixtureId, fix.name
            FROM fixture_by_function fbf
            JOIN room_fixture fix ON fbf.fixtureId = fix.fixtureId
            WHERE fbf.functionCode = ?
            ORDER BY fix.fixtureId
        """, (fallback_code,)).fetchall()
    ...
    return render_template('room_detail.html', 
                           room=room, beds=beds, fixtures=fixtures,
